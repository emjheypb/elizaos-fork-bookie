  ...
  handler: async (
    ...
    try {
      ...
      if (!ordersResponse) {
        if (callback) {
          await callback({
            text: `I'm having connection issues with Kalshi right now and can't pull your orders. The platform might be having hiccups - try again in a few minutes.`,
          });
        }
        return true;
      }
      ...
      if (callback) {
        await callback({
          text: `The connection to Kalshi is acting up and I can't get your orders right now. Could be API issues on their end. Give it a few minutes and try again - these platforms can be temperamental.`,
        });
      }
      return false;
    }
