...
const action: Action = {
  ...
  validate: async (_runtime: IAgentRuntime, message: Memory) => {
    ...
    return (isKalshiMarketRequest || isGeneralMarketSearch) && text.length > 3;
  },
  handler: async (
    ...
    try {
      ...
      if (!seriesResponse || !seriesResponse.series) {
        if (callback) {
          ...
          await callback({
            text: `${getRandomOpeningPhrase()} I'm having connection issues with Kalshi right now and can't pull the available markets. The platform might be having hiccups - try again in a few minutes. ${getRandomMarketInsight()}`,
          });
        }
        return true;
      }
      ...
      if (words.length === 0) {
        ...
        if (callback) {
          await callback({
            text: `${getRandomOpeningPhrase()} I couldn't find any specific trades to look for. Please provide more details or keywords related to the trades you want to see. ${getRandomMarketInsight()}`,
          });
        }
        return true;
      }
      ...
      if (filteredSeries.length === 0) {
        ...
        if (callback) {
          await callback({
            text: `${getRandomOpeningPhrase()} I couldn't find any active trades matching "${words}". ${getRandomMarketInsight()}`,
          });
        }
        return false;
      }
      ...
      for (const series of filteredSeries) {
        ...
        if (
          !event.events[0].markets ||
          event.events[0].markets.length === 0 ||
          !event.events[0].markets.find((market) => market.yes_bid > 0 && market.no_bid > 0)
        ) {
          logger.error(`No markets found for series: ${series.title} (${series.ticker})`);
          continue;
        }
      ...
      if (callback) {
        await callback({
          text: `${getRandomOpeningPhrase()} The connection to Kalshi is acting up and I can't get the available markets right now. Could be API issues on their end. Give it a few minutes and try again - these platforms can be temperamental. ${getRandomMarketInsight()}`,
        });
      }
      return false;
    }
